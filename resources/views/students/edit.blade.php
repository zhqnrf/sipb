...
@section('content')
<form method="post" action="{{ route('students.update',$student) }}" class="row g-3">
  @csrf @method('put')
  ...
  <div class="col-md-6">
    ...
    <select id="classroomSelect" name="classroom_id" class="form-select @error('classroom_id') is-invalid @enderror">
      ...
      @foreach($classrooms as $c)
        <option value="{{ $c->id }}" data-rombel="{{ $c->rombel }}"
          @if(old('classroom_id', $student->classroom_id) == $c->id) selected @endif>
          {{ $c->name }}{{ $c->rombel ? ' - '.$c->rombel : '' }}
        </option>
      @endforeach
    </select>
  ...
  <div class="col-md-6">
    ...
    <input type="text" name="rombel" id="rombelInput"
           ...
           value="{{ old('rombel', $student->rombel) }}" placeholder="Otomatis dari kelas, bisa diubah">
  ...
  <div class="col-12">
    ...
    <input type="text" name="kelas" class="form-control @error('kelas') is-invalid @enderror"
           value="{{ old('kelas', $student->kelas) }}" placeholder="Contoh: X IPA 1 (akan otomatis jika pilih kelas)">
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  if(sel && rom){
    // Isi otomatis saat load jika belum ada rombel/kelas (misal saat ganti kelas)
    @if(!old('rombel', $student->rombel))
      fillFromClass();
    @endif
